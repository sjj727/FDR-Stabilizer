multi.split <- function(x, y, B = 100, fraction = 0.5,
  ...
  oneSplit <- function(b) { # b-th split {e.g. called from *apply()
    ...
    while(try.again & (repeat.count <= repeat.max)) {
      ...
      if(p.sel > 0 && p.sel < nrow(x.right) - 1) {
        ...
        try(sel.pval <- do.call(classical.fit,
                                args = c(list(x = x.right[,sel.model],
                                              y = y.right), args.classical.fit)),silent = TRUE)
        ## Sanity checks for the output of classical.fit
        if(is.null(sel.pval)){
          r <- lm(y.right~x.right[,sel.model]-1)
          sel.model <- sel.model[which(is.na(r$coefficients)==0)]
          p.sel <- length(sel.model)
          sel.pval <- NULL
          try(sel.pval <- do.call(classical.fit,
                                  args = c(list(x = x.right[,sel.model],
      ...
      if(p.sel == 0) {
        ...
        try.again <- TRUE ## break the loop, continue with next sample-split
      }
...
MBHq <- function(X, y, q, num_split){
  p <- dim(X)[2]
  multi_fit = multi.split(X, y, B = num_split,return.nonaggr=T)
  ...
  if(length(cutoff)){
    ...
    MBH_fdp   <- result$fdp
    MBH_power <- result$power
    num_mbh <- length(selected_index)
  }else{
    MBH_fdp <- MBH_power <- 0
  }
  
  pvalues_nonaggr = multi_fit$pvals.nonaggr
  num <- 0
  e <- matrix(0,nrow = num_split,ncol = p)
  for (j in 1:num_split) {
    pvalues <- pvalues_nonaggr[j,]
    sorted_pvalues = sort(pvalues, decreasing = F, index.return = T)
    cutoff <- which(sorted_pvalues$x <= (1:p)*q)
    selected_index <- NULL
    if(length(cutoff)){
  ...
  BH_fdp_stab <- result_stab$fdp
  BH_power_stab <- result_stab$power
  result_bh <- fdp_power(selected_index, signal_index)
  BH_fdp <- result_bh$fdp
  BH_power <- result_bh$power
  
  return(list(fdp = MBH_fdp, power = MBH_power,
              BHfdp = BH_fdp, BHpower = BH_power,
              stab_fdp = BH_fdp_stab, stab_power = BH_power_stab,
              num_mbhq=num_mbh, num_stab=length(sel),num_bh=length(selected_index)))
}


MBHq_res <- function(X, y, q, num_split){
  p <- dim(X)[2]
  multi_fit = multi.split(X, y, B = num_split,return.nonaggr=T,ci=FALSE)
  pvalues = multi_fit$pval.corr
  sorted_pvalues = sort(pvalues, decreasing = F, index.return = T)
  cutoff <- which(sorted_pvalues$x <= (1:p)*q)
  if(length(cutoff)){
    cutoff <- max(cutoff)
    selected_index <- sorted_pvalues$ix[1:cutoff]
    MBHq_selected_index <- selected_index
  }
  else{MBHq_selected_index <- NULL}
  pvalues_nonaggr = multi_fit$pvals.nonaggr
  
  num <- 0
  e <- matrix(0,nrow = num_split,ncol = p)
  for (j in 1:num_split) {
    pvalues1 <- pvalues_nonaggr[j,]
    sorted_pvalues = sort(pvalues1, decreasing = F, index.return = T)
    cutoff <- which(sorted_pvalues$x <= (1:p)*q)
    selected_index <- NULL
    if(length(cutoff)){
      cutoff <- max(cutoff)
  ...
  return(list(stab_sel = sel, e_avg = e_avg, MBHq_sel = MBHq_selected_index,BHq_sel=selected_index,
              pvalues_nonaggr = pvalues_nonaggr))
}
